...
class APIAnalyzer:
    ...
    def checkSubmittedAPI(self):
        ...
        for api in APIs:
            ...
            conn = self.dbConnection.get_db_connection()
            cur = conn.cursor()
            cur.execute("update api_security_check_tbl set low_count=%s,mid_count=%s,high_count=%s,report=%s,completed_timestamp=current_timestamp at time zone 'UTC' where scan_id = %s and status=1;",(low_count,mid_count,high_count,str(apiObj),api[3]))
            conn.commit()
            cur.close()
            conn.close()
            low_count = 0
